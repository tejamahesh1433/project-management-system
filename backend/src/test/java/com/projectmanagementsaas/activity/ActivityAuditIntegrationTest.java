package com.projectmanagementsaas.activity;
...
class ActivityAuditIntegrationTest extends IntegrationTestBase {
    @Autowired
    ...
    @Test
    void existingEventsCreateActivityAndAuditRecords() throws Exception {
        UserSession owner = register("activity-owner@example.com");
        ProjectSetup setup = createProjectWithWorkspace(owner.token(), "activity-flow");
        UUID taskId = createTask(owner.token(), setup.projectId());
        ...
        mockMvc.perform(get("/api/v1/activity/workspaces/{workspaceId}", setup.workspaceId())
                        .header(HttpHeaders.AUTHORIZATION, bearer(owner.token())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].workspaceId").value(setup.workspaceId().toString()));

        mockMvc.perform(get("/api/v1/activity/projects/{projectId}", setup.projectId())
                        .header(HttpHeaders.AUTHORIZATION, bearer(owner.token())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].projectId").value(setup.projectId().toString()));

        mockMvc.perform(get("/api/v1/audit")
                        .header(HttpHeaders.AUTHORIZATION, bearer(owner.token()))
                        .header(HttpHeaders.USER_AGENT, "phase8-test"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].actorId").value(owner.userId().toString()));

        mockMvc.perform(get("/api/v1/audit/{entityType}/{entityId}", "TASK", taskId)
                        .header(HttpHeaders.AUTHORIZATION, bearer(owner.token())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].entityType").value("TASK"))
                .andExpect(jsonPath("$[0].entityId").value(taskId.toString()));
    }
    ...
    private UUID createTask(String token, UUID projectId) throws Exception {
        String response = mockMvc.perform(post("/api/v1/tasks")
                        ...
                        .content(objectMapper.writeValueAsString(new CreateTaskRequest(
                                projectId, null, "Activity task", null, TaskPriority.MEDIUM, TaskType.TASK, null, null, 0))))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
